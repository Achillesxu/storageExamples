...
func getMetadata(name string, versionId int) (meta Metadata, e error) {
	u := fmt.Sprintf("http://%s/metadata/objects/%s_%d/_source",
...
func SearchLatestVersion(name string) (meta Metadata, e error) {
	u := fmt.Sprintf("http://%s/metadata/_search?q=name:%s&size=1&sort=version:desc",
...
func PutMetadata(name string, version int, size int64, hash string) error {
	...
	client := http.Client{}
	u := fmt.Sprintf("http://%s/metadata/objects/%s_%d?op_type=create",
		os.Getenv("ES_SERVER"), name, version)
	request, _ := http.NewRequest("PUT", u, strings.NewReader(doc))
...
func SearchAllVersions(name string, from, size int) ([]Metadata, error) {
	u := fmt.Sprintf("http://%s/metadata/_search?sort=name,version&from=%d&size=%d",
...
func DelMetadata(name string, version int) {
	client := http.Client{}
	u := fmt.Sprintf("http://%s/metadata/objects/%s_%d",
		os.Getenv("ES_SERVER"), name, version)
	request, _ := http.NewRequest("DELETE", u, nil)
...
func SearchVersionStatus(minDocCount int) ([]Bucket, error) {
	client := http.Client{}
	u := fmt.Sprintf("http://%s/metadata/_search", os.Getenv("ES_SERVER"))
	body := fmt.Sprintf(`
        {
          "size": 0,
          "aggs": {
	...
	request, _ := http.NewRequest("GET", u, strings.NewReader(body))
	r, e := client.Do(request)
	if e != nil {
		return nil, e
	}
...
func HasHash(hash string) (bool, error) {
	u := fmt.Sprintf("http://%s/metadata/_search?q=hash:%s&size=0", os.Getenv("ES_SERVER"), hash)
	r, e := http.Get(u)
	if e != nil {
		return false, e
	}
...
func SearchHashSize(hash string) (size int64, e error) {
	u := fmt.Sprintf("http://%s/metadata/_search?q=hash:%s&size=1",
